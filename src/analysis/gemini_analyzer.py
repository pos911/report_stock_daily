...
2. `1) 공격적인 포인트`: 가격 모멘텀과 이동평균을 먼저 보고, 수급은 확인 신호로만 해석하라. 외국인 매도 중 가격 상승은 부정 단정이 아니라 흡수 강도로 해석하라.
3. `2) 최대한 보수적인 포인트`: 하방 리스크, 매크로 민감도, 변동성, 밸류 부담을 시간축별로 분리해 서술하라.
4. `3) 최종 결론`: 동일 업종 내 종목은 같은 기준으로 BUY/HOLD/SELL을 판단하라. 특히 삼성전자와 SK하이닉스처럼 같은 반도체는 모멘텀/밸류/수급 기준을 일관되게 비교하라.
5. `4) 실행 전략`: 비중 확대/축소, 분할 매수, 추격 금지, 관찰 가격/팩터 조건, 손절 또는 리스크 관리 기준 중 최소 2개를 제시하라.
