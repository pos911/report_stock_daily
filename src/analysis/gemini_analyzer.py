...
"""
    ...
    def _compact_target_stocks_data(self, target_stocks_data):
        ...
        for row in target_stocks_data.get("normalized_stock_prices_daily", []) or []:
            if not isinstance(row, dict):
                continue
            symbol = row.get("symbol")
            if not symbol:
    ...
    @staticmethod
    def _build_quant_decision_rules() -> str:
        return """
## 정량 해석 원칙
- 현재 신호 체계는 `signal_model_version=v0.1_unbacktested` 휴리스틱이다. 확정적 매수/매도 추천처럼 단정하지 말고 `신호`, `확신도`, `리스크 관리 메모`로 표현한다.
- 단일 서사 금지: 유가, 환율, 금리, VIX, SOX, 국내 지수, 수급이 서로 충돌하면 `팩터 분화` 또는 `혼재`로 표현하고 Risk-On/Risk-Off 한쪽으로 억지 압축하지 마라.
- 결론 선결정 금지: 긍정 신호와 부정 신호를 먼저 나열한 뒤 결론을 도출한다. `결론 → 근거 맞추기` 방식 금지.
- 가격 우선 원칙: 개별 종목은 가격/수익률/이동평균/변동성이 1차 신호이고, 외국인·기관 수급은 확인 신호다. 외국인 매도 중 가격 상승은 무조건 부정이 아니라 `공급 흡수`, `숏커버/리밸런싱 가능성`, `가격 주도 강세`로 해석한다.
- 수급 해석: 외국인 매도는 헤지, ETF 리밸런싱, 차익실현일 수 있다. 가격 반응과 함께 해석하지 않은 `외국인 매도=악재` 표현 금지.
- 밸류 해석: PER/PBR 절대값만으로 저평가/고평가를 단정하지 마라. 플랫폼, 반도체, 금융, 통신, 소비재 등 업종/사이클에 따라 의미가 다르며, 업종 기준 데이터가 없으면 `절대 배수 기준의 제한적 해석`이라고 짧게 표현한다.
- 시간축 분리: 1일 변화, 5일 모멘텀, 20일 변동성/이동평균, 중장기 밸류를 한 문장에 섞어 결론 내리지 말고 각각 다른 타임프레임으로 구분한다.
- ETF 거래대금: 인버스/레버리지/테마 ETF 거래대금 증가는 방향성 예측이 아니라 단기 트레이딩과 변동성 확대 신호로 해석한다.
- 거시-종목 연결: 모든 종목에 같은 매크로 프레임을 적용하지 마라. 환율, 유가, 금리, 반도체 지수, 소비 심리 등 종목별 민감도가 있는 경우에만 연결한다.
- 행동 전략 필수: 시장/종목 판단에는 `비중`, `추격/분할`, `손절 또는 리스크 관리 기준`, `관찰 조건` 중 최소 2개를 포함한다.
"""
...
{self._build_quant_decision_rules()}
...
3. KOSPI/KOSDAQ 값은 `kospi`, `kosdaq` 키가 있을 때만 작성하라. S&P 500, Nasdaq, SOX 같은 미국 지수 값을 KOSPI/KOSDAQ 위치에 절대 대입하지 마라.
4. 개인/외국인/기관 수급은 `*_individual_net_buy`, `*_foreign_net_buy`, `*_institutional_net_buy` 키가 있을 때만 작성하라.
5. 수치 나열 직후 `**[시장 평가]**` 항목을 추가하여, 현재 변화율이 주는 종합적인 의미를 한 줄로 요약하라.
6. `### 오늘의 시장 판단`에는 현재 시장이 `Risk-On` 인지 `Risk-Off` 인지 혹은 `중립/팩터 분화`인지 명시하라.
7. SOX 상승·VIX 하락 같은 위험선호 신호와 환율·유가·금리 부담이 충돌하면 `중립/팩터 분화`로 판단하고, 어느 팩터가 어느 섹터에 유리/불리한지 짧게 설명하라.

{self._build_silent_skip_rules(blocked_sections)}
마크다운 형식으로 작성해줘.
"""
...
3. `### 투자 시사점`은 제공된 StockData의 모멘텀, 수급, 밸류에이션, 거래대금, 변동성 데이터를 투자공학 관점에서 분석하고, 위 핵심 포인트에서 뉴스와 종합한 내용을 기초로 2~3개만 제시하라.
4. 데이터 품질 문제는 별도 본문 섹션으로 만들지 말고, 꼭 필요한 경우 마지막 bullet에 짧게만 반영하라.
5. 새 사실을 지어내지 말고, 입력에 없는 종목/수치는 추가하지 마라.
6. 마지막 bullet에는 가능한 경우 `비중 조절`, `섹터 로테이션`, `추격 매수 자제/분할 접근`, `리스크 관리 기준` 중 하나 이상의 행동 전략을 포함하라.
...
"""
    ...
    def summarize_news_context(self, news_text: str, report_type: str = "regular", blocked_sections: list[str] | None = None) -> str:
        """
...
3. ETF, 인버스, 레버리지 상품의 거래대금 증가는 방향성 예측이 아니라 변동성/단기 트레이딩 확대 신호로 해석하라.
4. `foreign_flow_zscore`, 수익률, 밸류에이션 중 의미 있는 근거만 짧게 포함하라. 단, 근거 없는 추정은 금지한다.
5. `Report 작성 시간`, `### 매크로 분석`, `### 최종 투자 전략` 같은 추가 섹션은 절대 쓰지 마라.
...
2. `2) 수급/거래 신호`
3. `3) 리스크/밸류 신호`
4. `4) Signal Score / Confidence / 신호 라벨`
5. `5) 전략 메모`

### 작성 규칙
1. 종목명(코드) 옆에는 제공된 가격/수익률/모멘텀 지표만 표기하라. return_1d가 없으면 억지로 만들지 말고 5일 수익률이나 종가 중심으로 쓴다.
2. `1) 가격 신호`: 가격 모멘텀과 이동평균을 먼저 보고, 수급은 확인 신호로만 해석하라.
3. `2) 수급/거래 신호`: 외국인/기관/개인 수급, 거래량 rank, 거래대금 rank, 거래대금 집중 여부를 적는다.
4. `3) 리스크/밸류 신호`: 하방 리스크, 매크로 민감도, 변동성, 공매도, 밸류 부담을 시간축별로 분리해 서술하라.
5. `4) Signal Score / Confidence / 신호 라벨`: 확정적 매수/매도/HOLD 대신 `비중확대 후보`, `보유/관찰`, `관망`, `리스크 축소 후보`, `판단 유보` 중 하나를 사용한다.
6. `5) 전략 메모`: 비중 확대/축소, 분할 접근, 추격 금지, 관찰 조건, 리스크 관리 기준 중 최소 2개를 제시하라.
6. 장문 서론은 금지하고 바로 종목 핵심만 적어라.
...
"""
    ...
    def generate_morning_us_summary(self, market_data: dict, news_context: str, blocked_sections: list[str] | None = None) -> str:
        prompt = f"""
[미국 시장 수치]
{json.dumps(market_data or {}, ensure_ascii=False)}

[해외 뉴스 입력]
{news_context or ""}

[System Instruction]
반드시 3줄만 작성하라. 각 줄은 1문장으로 끝내고, 미국 증시 방향, 금리/달러/원자재, 핵심 뉴스의 시장 함의를 요약하라.
숫자는 입력값만 사용하고, 없는 값은 언급하지 마라.
- 미국 10년물과 3년물, 10Y-3Y 스프레드가 있으면 짧게 해석하라.
- 금리차만으로 경기침체를 단정하지 말고 금리 레벨, 달러, VIX, 지수 흐름과 함께 해석하라.
헤더, 인사말, 결론, 불필요한 수식어는 금지한다.

{self._build_silent_skip_rules(blocked_sections)}
"""
        return self._call_model(prompt, temperature=0.3).strip()

    def summarize_news_reason(self, stock_name: str, news_items: list[dict], blocked_sections: list[str] | None = None) -> str:
        if not news_items:
            return ""
        compact_news = [
            {
...
[System Instruction]
위 뉴스만 근거로 해당 종목이 왜 시장 관심을 받을 수 있는지 1문장 한국어로 요약하라.
반복되는 핵심 테마가 보이면 그 테마를 먼저 요약하고, 단순히 "관심이 집중됐다" 같은 상투 문구만 남기지 마라.
가격, 수급, 환율, 지수 숫자는 새로 쓰지 마라. 뉴스에 없는 사실도 만들지 마라.

{self._build_silent_skip_rules(blocked_sections)}
"""
        return self._call_model(prompt, temperature=0.2).strip()

    def generate_top_volume_reason(self, context: dict, blocked_sections: list[str] | None = None) -> str:
        prompt = f"""
[종목/상품 컨텍스트]
{json.dumps(context, ensure_ascii=False)}

[System Instruction]
아래 정보만 바탕으로 해당 종목 또는 ETF/ETN이 오늘 왜 주목받는지 1문장으로만 요약하라.
- market, asset_type, rank_type, rank는 입력값을 그대로 사용하고 새로 추론하거나 바꾸지 마라.
- 가격, 거래량, 거래대금 수치는 입력값만 참고하고 새 숫자를 만들지 마라.
- 뉴스가 있으면 뉴스와 테마를 우선 반영하라.
- ETF/ETN이면 개별 기업보다 기초자산, 테마, 변동성 맥락을 우선 설명하라.
- OpenDart event가 있으면 보조 근거로만 짧게 반영하라.
- 관련 뉴스가 부족하면 종목명/상품명에서 테마를 추론하되 동어반복 문장은 피하라.
- 반드시 한 문장만 출력하라.
