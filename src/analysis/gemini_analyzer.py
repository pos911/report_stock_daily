...
"""
    ...
    def generate_stock_analysis(
        ...
        """
        [Step 2] Top-volume names + focus-stock analysis.
        """
...
[System Instruction]
거래대금 상위 종목과 관심 종목을 한 번에 매우 실용적으로 정리하라.

### 출력 구조
- `### 거래대금 상위 종목`
- 상위 종목 bullet: `- 종목명(코드): 한 줄 요약`
- `### 관심 종목 분석`
- 종목별 고정 형식:
1. `1) 공격적인 포인트`
2. `2) 최대한 보수적인 포인트`
...
### 작성 규칙
1. 거래대금 상위 종목은 눈에 띄는 이름만 짧게 요약하라.
2. 관심 종목의 공격적인 포인트에는 모멘텀, 수급, 밸류에이션, 업황 중 강점만 압축해 적어라.
3. 보수적인 포인트에는 리스크, 데이터 지연 가능성, 업황 역풍을 적어라.
4. 결론은 종목마다 하나만 명시하라.
5. 종목별 장문 서론은 금지하고 바로 핵심만 적어라.
